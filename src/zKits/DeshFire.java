...
public class DeshFire implements Listener, CommandExecutor
{
    ...
    public void interact(final PlayerInteractEvent e) {
        ...
        if ((e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && Arrays.desh.contains(p.getName()) && p.getItemInHand().getType() == Material.REDSTONE_BLOCK) {
            e.setCancelled(true);
            p.updateInventory();
            if (DeshFire.cooldownm.contains(p)) {
                p.sendMessage(String.valueOf(Main.getPlugin().getConfig().getString("server").replace("&", "§")) + ">> §eKit em Cooldown!");
                return;
            }
            ...
            p.sendMessage(String.valueOf(Main.getPlugin().getConfig().getString("server").replace("&", "§")) + ">>  §elDeshFire Usado!");
