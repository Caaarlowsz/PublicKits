...
public class ScoreB implements Listener
{
    public static void setScoreBoard(final Player p) {
        ...
        o.setDisplayName(new StringBuilder().append(Main.getPlugin().getConfig().getString("server").replace("&", "§")).toString());
        final Score espaco3 = o.getScore(Bukkit.getOfflinePlayer("§2"));
        final Score kills = o.getScore(Bukkit.getOfflinePlayer("§7Matou§6>>"));
        final Score killsr = o.getScore(Bukkit.getOfflinePlayer("§a" + Main.getPlugin().getConfig().getInt("status." + p.getName().toLowerCase() + ".kills")));
        final Score deaths = o.getScore(Bukkit.getOfflinePlayer("§7Morreu§6>>"));
        final Score deathsr = o.getScore(Bukkit.getOfflinePlayer("§c" + Main.getPlugin().getConfig().getInt("status." + p.getName().toLowerCase() + ".mortes")));
